...
namespace Spectra::Math
{
	...
	public:
		...
		Quaternion(const float& w, const Vector3<T>& v)
			: w(w)
			, v(v)
		{
		}
		...
		Quaternion operator*(const Quaternion& q2) const
		{
			return Quaternion(
				...
				Vector3<T>(w * q2.v.x + v.x * q2.w + v.y * q2.v.z - v.z * q2.v.y,
		...
		{
			Quaternion conjugate(w, Vector3<T>(-(v.x), -(v.y), -(v.z)));
		...
		T DotProduct(const Quaternion& q1, const Quaternion& q2) const
		{
			return q1.w * q2.w + q1.v.x * q2.v.x + q1.v.y * q2.v.y + q1.v.z * q2.v.z;
		}

		Quaternion& Exp(const float scalar) const
		{
			if (fabs(w) < .9999f) {
				float alpha = ACos(w);
		...
		Quaternion FromMatToQuat(const Mat4x4<T>& m)
		{
